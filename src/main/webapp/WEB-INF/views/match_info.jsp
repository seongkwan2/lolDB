...
<body>
	...
	<c:set var="perkPath"
		value="https://ddragon.leagueoflegends.com/cdn/img/" />
	<div class="record_container">
		<c:forEach items="${MatchList}" var="match">
			<div class="game-header"
				id="${match.info.teams[0].win ? 'winback' : 'lossback'}">
					...
					<c:if test="${participant.summonerName eq UserInfo.name}">
						...
						<div class="game-header-element" id="bar-champ">
							<c:forEach var="champion" items="${champions}">
								<c:if test="${participant.championId eq champion.key}">
									<c:set var="imagePath"
										value="${imageBasePath}${champion.image.full}" />
									<img src="${imagePath}" alt="${champion.name}"
										id="record-champ-image">
								</c:if>
							</c:forEach>
						</div>
						<div class="game-header-element" id="bar-level">
							Lv.${participant.champLevel}</div>
						<div class="game-header-element">
							<div>
								<c:forEach var="spell" items="${spells}">
									<c:if
										test="${participant.summoner1Id eq spell.key or participant.summoner2Id eq spell.key}">
										<c:set var="spellPaths"
											value="${spellPath}${spell.image.full}" />
							...
							<div>
								<c:forEach var="runes" items="${runes}">
									<%-- 핵심 룬 --%>
									<c:forEach var="rune" items="${runes.slots[0].runes}"
										varStatus="loop">
										<c:if
											test="${participant.perks.styles[0].selections[0].perk eq rune.id}">
											<img src="${perkPath}${rune.icon}"
												alt="Perk Image ${loop.index}" id="rune-image" />
										</c:if>
									</c:forEach>
									<%-- 보조 룬 --%>
									<c:if test="${participant.perks.styles[1].style eq runes.id}">
										<img src="${perkPath}${runes.icon}" alt="Perk Image"
											id="rune-image" />
									</c:if>
								</c:forEach>
							</div>
						...
						<div class="game-header-element" id="bar-item">
							<c:forEach var="item" items="${itemList}">
								<c:if test="${item ne '0'}">
									<img
										src="https://ddragon.leagueoflegends.com/cdn/13.22.1/img/item/${item}.png"
										alt='item Image' id="bar-item-image">
								</c:if>
							</c:forEach>
						</div>

						<div class="game-header-element" id="bar-kda">${participant.kills}
							/ ${participant.deaths} / ${participant.assists}</div>

						<div class="game-header-element" id="bar-goldcs">
							${participant.goldEarned}G <br>
						...
						<div class="game-header-element" id="bar-ward">
							...
							${participant.visionWardsBoughtInGame}<br>
				...
				<div
					class="${match.info.teams[0].win ? 'win' : 'loss'} game-header-element"
					id="bar-winloss">${match.info.teams[0].win ? 'WIN' :
                      'LOSE'}</div>
			...
			<div class="game-details" style="display: none;">
							...
							<c:forEach items="${match.info.participants}" var="participant">
								<tr class="participant-row">
									<c:if test="${participant.teamId == 100}">
										<c:set var="champPath"
											value="${champIconPath}${participant.championName}.png" />
										<c:set var="itemList"
											value="${participant.item0},${participant.item1},${participant.item2},${participant.item3},${participant.item4},${participant.item5},${participant.item6}" />

										<td><c:forEach var="champion" items="${champions}">
												<c:if test="${participant.championId eq champion.key}">
													<c:set var="imagePath"
														value="${imageBasePath}${champion.image.full}" />
													<img src="${imagePath}" alt="${champion.name}"
										...
										<td>${participant.kills}/ ${participant.deaths} /
											${participant.assists}</td>
										<td>
											<div>
												<c:forEach var="spell" items="${spells}">
													<c:if
														...
														<c:set var="spellPaths"
															value="${spellPath}${spell.image.full}" />
														<img src="${spellPaths}" alt="${spell.name}"
															id="spell-image">
													</c:if>
											...
											<div>
												<c:forEach var="runes" items="${runes}">
													...
													<%-- 보조 룬 --%>
													<c:if
														test="${participant.perks.styles[1].style eq runes.id}">
														<img src="${perkPath}${runes.icon}" alt="Perk Image"
															id="rune-image" />
													</c:if>
												</c:forEach>
										...
										<td>${participant.goldEarned}G<br>
											${participant.totalMinionsKilled + participant.neutralMinionsKilled}CS
										</td>
										<td>${participant.totalDamageDealt}</td>
										<td>${participant.totalDamageTaken}</td>
										<td><img src="/images/pinkward.png" alt="pinkward"
											id="pinkward"> ${participant.visionWardsBoughtInGame}<br>${participant.wardsPlaced}
				...
				<!-- 레드 팀 -->
				<div class="red-team"
					id="${match.info.teams[1].win ? 'winback' : 'lossback'}">
					<h4>RED Team</h4>
					<table>
						<thead>
							<tr>
